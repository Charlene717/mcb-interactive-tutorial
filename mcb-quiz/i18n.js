/* Molecular & Cellular Biology Quiz · i18n
 * - localStorage key 'mcb_quiz_lang'
 * - data-lang block toggling, data-zh/data-en textContent swapping
 * - Dispatches 'langchange' CustomEvent so dynamic renderers (renderQuiz) refresh
 */
(function(){
  const SK = 'mcb_quiz_lang';
  let lang = localStorage.getItem(SK) || 'en';

  // UI string dictionary used by JS dynamic rendering
  const STR = {
    appTitle:        { zh:"🧬 分子與細胞生物學 互動考題", en:"🧬 Molecular & Cellular Biology Interactive Quiz" },
    ...
    rangeAll:        { zh:"全部 20 個章節", en:"All 20 chapters" },
    ...
    quizAllChapters: { zh:"分子與細胞生物學 互動考題（全部章節）", en:"Molecular & Cellular Biology Interactive Quiz (All Chapters)" },
    quizErrataTitle: { zh:"分子與細胞生物學 互動考題", en:"Molecular & Cellular Biology Interactive Quiz" },
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    ...
    if(host && !document.getElementById('langToggle')){
      ...
      btn.style.cssText = 'background:#fff;color:#6d28d9;border:1px solid rgba(255,255,255,.5);border-radius:999px;padding:5px 14px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;white-space:nowrap';
